...
getDirectories('./commands', function (err, res) {
  if (err) {console.log('Error', err); return;};
   ...
   fileCollection = res.map(element => element.endsWith('js')? element : undefined).filter(Boolean)
   ...
   */
    ...
    displayOptions : function(wantsLog = {consoleCommands: false, consoleRAM : false, customMessage: false}) {
  ...
  */
    ...
    if(command.withMultipleArguments){
     ...
     return statTypedArgument = statTypedArgument.map(el =>  el = desiredType(el))
...
for(let aPermission of allValidPermissions){
      if(!member.permissions.has(aPermission)){
        ...
        message.reply(`You do not have the correct permissions to use ${command.name}`)
        return
      }
    }
